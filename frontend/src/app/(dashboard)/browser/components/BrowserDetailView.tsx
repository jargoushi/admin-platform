...
export function BrowserDetailView({ data }: BrowserDetailViewProps) {
    ...
    const fingerPrintItems = [
        ...
        { label: '窗口分辨率', value: `${fingerPrint.openWidth} x ${fingerPrint.openHeight}` },
    ...
    return (
        <div className='space-y-6 py-4'>
            <section>
                ...
                <div className='grid grid-cols-2 gap-4 rounded-lg border p-4 bg-muted/30'>
                    {detailItems.map((item, index) => (
                        <div key={index} className={`space-y-1 ${item.label === 'User Agent' ? 'col-span-2' : ''}`}>
            ...
            <section>
                <h3 className='mb-3 text-sm font-medium text-muted-foreground'>指纹配置</h3>
                <div className='grid grid-cols-2 gap-4 rounded-lg border p-4 bg-muted/30'>
                    {fingerPrintItems.map((item, index) => (
                        <div key={index} className={`space-y-1 ${item.fullWidth ? 'col-span-2' : ''}`}>
                            <div className='text-xs text-muted-foreground'>{item.label}</div>
